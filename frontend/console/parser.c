...
/* Function that parses a text string and returns
   a NULL separated string with the translated result.
*/
static char* parse(const char* text) {
  ...
  for(i = 0; i < len; i++) {
	...
	lprintf(LOG_ERROR, "Unknown escape code received in parser.\n");
      }
    } else {
      if(text[i] == '\\')
	mode |= PARSER_ESCAPE;
...
static void print_help(const parser_command* cmds, const char* cmd) {
  ...
  if(!cmd) {
	...
	if(cmds[i].help_long)
	  ...
	  lprintf(LOG_NORMAL, "No help availible for '%s'.\n", cmd);
...
void parser_exec(const parser_command* cmds, void* data, const char* c) {
  ...
  for(i = 0; cmds[i].cmd_name; i++) {
	...
	lprintf(LOG_ERROR, "Invalid number of parameters for '%s'\n",
		cmds[i].cmd_name);
